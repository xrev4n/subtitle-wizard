...
export default function SubtitlePreview({
  ...
  const renderStatusBadge = (sub) => {
    switch (sub.status) {
      case 'translating':
        return (
          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-indigo-500/15 text-indigo-300 border border-indigo-500/30 animate-pulse">
      ...
      case 'editing':
        return (
          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-cyan-500/10 text-cyan-300 border border-cyan-500/20">
      ...
      default:
        return (
          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-slate-800 text-slate-400 border border-white/5">
  ...
  if (!subtitles || subtitles.length === 0) {
    return (
      <div className="glass-card rounded-3xl p-10 border border-white/10 text-center">
  ...
  return (
    <div className="glass-card rounded-3xl border border-white/10 overflow-hidden space-y-0 shadow-xl">
      
      {/* Top action header */}
      <div className="p-4 sm:p-5 border-b border-white/10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 bg-slate-950/60">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-sm sm:text-base font-bold text-white flex items-center gap-2 m-0">
            ...
            {integrity.totalIssues > 0 && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-amber-500/15 text-amber-300 border border-amber-500/30 animate-pulse">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto justify-end">
          ...
          <div className="relative flex-1 sm:w-44">
            ...
            <input
              ...
              className="w-full pl-8 pr-2.5 py-1.5 text-xs bg-slate-950 border border-white/10 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
            />
          </div>
          ...
          <button
            ...
            className="px-2.5 py-1.5 text-xs font-semibold rounded-xl bg-cyan-600/15 hover:bg-cyan-600/25 text-cyan-300 border border-cyan-500/30 transition-all flex items-center gap-1.5 cursor-pointer"
          >
          ...
          {canUndo && (
            <button
              type="button"
              onClick={onUndo}
              className="px-2.5 py-1.5 text-xs font-semibold rounded-xl bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 border border-amber-500/30 transition-all flex items-center gap-1.5 cursor-pointer"
              title={t('editor.undo')}
            >
          ...
          <button
            ...
            className="px-3 py-1.5 text-xs font-bold rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white shadow-md shadow-indigo-600/25 transition-all flex items-center gap-1.5 cursor-pointer"
          >
      ...
      <div className="flex border-b border-white/10 px-4 sm:px-5 bg-slate-950/40 gap-2 overflow-x-auto items-center justify-between">
        <div className="flex gap-2">
          <button
            ...
            className={`py-2 px-2.5 text-xs font-semibold border-b-2 transition-colors flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
              statusFilter === 'all'
          ...
          filteredSubtitles.map((sub, idx) => {
            ...
            return (
              <div
                key={sub.id}
                id={`cue-row-${sub.id}`}
                className={`p-3.5 sm:p-4 transition-all flex flex-col md:flex-row items-start gap-3.5 text-left ${
                  isCurrentlyPlaying
              ...
              >
                ...
                <div className="md:w-48 flex-shrink-0 space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      ...
                      />
                      <span className="w-5 h-5 rounded bg-slate-800 border border-white/10 text-[10px] font-mono font-bold text-slate-300 flex items-center justify-center">
                        {sub.id}
                      </span>
                      {isCurrentlyPlaying && (
                        <span className="p-0.5 rounded bg-indigo-500 text-white animate-pulse">
                  ...
                  <div className="bg-slate-950 p-1.5 rounded-xl border border-white/10 space-y-1">
                    <div className="flex items-center justify-between text-[10px] font-mono text-slate-300">
                      <input
                        type="text"
                        value={sub.startTime}
                        onChange={(e) => onUpdateCue(sub.id, { startTime: e.target.value })}
                        className="w-18 bg-slate-900 px-1 py-0.5 rounded border border-white/10 text-white font-mono text-[11px] focus:outline-none focus:border-indigo-500"
                  ...
                  {cueIssues.length > 0 && (
                    <div className="space-y-1">
                      {cueIssues.map((issue, issueIdx) => (
                        <div
                          key={issueIdx}
                          className="p-1 rounded bg-amber-500/15 border border-amber-500/30 text-amber-200 text-[9px] leading-tight flex items-start gap-1"
                        >
                  ...
                  <div className="flex items-center gap-1 pt-0.5">
                    ...
                    <button
                      ...
                      className="p-1 rounded bg-rose-500/10 hover:bg-rose-500/25 text-rose-300 transition-colors cursor-pointer"
                    ...
                    {sub.status === 'error' && onRetrySingleCue && (
                      <button
                        ...
                        className="p-1 rounded bg-amber-500/20 text-amber-300 hover:bg-amber-500/30 transition-colors cursor-pointer ml-auto"
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-2.5 w-full">
                  
                  {/* Source text in-place editable card */}
                  <div className="bg-slate-950/60 rounded-xl p-3 border border-white/5 flex flex-col justify-between focus-within:border-indigo-500/50 transition-colors">
                    <div>
                      <div className="text-[9px] font-semibold uppercase tracking-wider text-slate-500 mb-1 flex items-center justify-between">
                        <span>{t('parser.preview.sourceText')}</span>
                        <span className="text-[9px] text-slate-600 font-mono">Editable</span>
                      </div>
                      <textarea
                        rows={2}
                        ...
                        className="w-full bg-transparent text-xs text-slate-200 font-normal leading-relaxed resize-none focus:outline-none"
                      />
                    </div>
                  </div>
                  ...
                  <div
                    className={`rounded-xl p-3 border flex flex-col justify-between transition-all focus-within:border-indigo-500/70 ${
                  ...
                  >
                    <div>
                      <div className="text-[9px] font-semibold uppercase tracking-wider mb-1 flex items-center justify-between">
                      ...
                      <textarea
                        ...
                        placeholder={
                          sub.status === 'translating'
                            ...
                            : sub.status === 'error'
                            ? 'Error en traducción. Haz clic para escribir...'
                            : 'Traducción o texto destino...'
                        }
                        className="w-full bg-transparent text-xs text-indigo-100 font-normal leading-relaxed resize-none focus:outline-none placeholder-slate-600"
                      />
                    </div>
                  </div>
